...
show_menu() {
    ...
    echo "  8) ⚙️  Настройки (выбор папки конфигов)"
    echo "  9) 📚 Документация"
    echo "  q) Выход"
    echo
    ...
    if [ -n "$total" ] && [ "$total" -gt 0 ]; then
        if [ "$CONFIG_MODE" = "channel-widths" ]; then
            echo -e "  ${BOLD}Профили (ширина каналов):${NC} 01–${total}   ${BOLD}Список:${NC} опция 2"
        else
            echo -e "  ${BOLD}Профили (безопасность):${NC} 01–${total}   ${BOLD}Список:${NC} опция 2"
        fi
    fi
    echo
